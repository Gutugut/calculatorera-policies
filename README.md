# Calculator Era - Privacy Policy & Terms of Service

> 📄 **Official legal documents for Calculator Era iOS app**  
> 🇺🇸 **Published for US App Store compliance**

---

## 📱 About Calculator Era

**Calculator Era** is a powerful scientific calculator app for iOS, designed for the US market with:
- ✨ Advanced mathematical calculations
- 📊 Comprehensive calculation history
- 🔢 Unit conversions
- 🎨 Beautiful, modern interface
- 🚀 Fast and reliable performance

**Download**: [Calculator Era on App Store](https://apps.apple.com/app/id6739860116)

---

## 📄 Legal Documents

This repository hosts the privacy policy and terms of service for Calculator Era:

### 🔒 Privacy Policy
- **Live URL**: https://gutugut.github.io/calculatorera-policies/privacy.html
- **Last Updated**: November 23, 2025
- **Compliance**: GDPR, CCPA, COPPA, App Store Guidelines

### 📋 Terms of Service
- **Live URL**: https://gutugut.github.io/calculatorera-policies/terms.html
- **Last Updated**: November 23, 2025
- **Coverage**: License, purchases, liability, Apple App Store terms

### 🏠 Homepage
- **Live URL**: https://gutugut.github.io/calculatorera-policies/
- Quick access to all legal documents

---

## 🔐 Privacy Commitment

Calculator Era is committed to protecting user privacy:

### ✅ What We Do
- Collect anonymous usage data (with your consent)
- Use Firebase for crash reporting and analytics
- Display ads via Google AdMob (can be removed with in-app purchase)
- Follow industry best practices for data security

### ❌ What We DON'T Do
- We **DO NOT** sell your personal information
- We **DO NOT** collect identifiable personal data
- We **DO NOT** access your contacts or location
- We **DO NOT** share data without your consent

### 🎛️ Your Control
- Opt out of analytics in app settings anytime
- Control ad preferences in iOS settings
- Request data deletion anytime
- Exercise all your privacy rights under US law

---

## 📊 Third-Party Services

Calculator Era uses the following trusted services:

| Service | Purpose | Privacy Policy |
|---------|---------|----------------|
| **Firebase Analytics** | Usage tracking (optional) | [Google Firebase Privacy](https://firebase.google.com/support/privacy) |
| **Firebase Crashlytics** | Crash reporting | [Google Firebase Privacy](https://firebase.google.com/support/privacy) |
| **Google AdMob** | Advertisements | [AdMob Privacy](https://support.google.com/admob/answer/6128543) |

All services comply with Apple's App Store guidelines and US privacy laws including CCPA.

---

## 📧 Contact Information

### Support & Questions
- **Email**: enbilex@gmail.com
- **In-App**: Use the feedback feature in Calculator Era
- **Response Time**: Within 24-48 hours (US business hours)

### Privacy Requests
To exercise your privacy rights under CCPA (access, correction, deletion):
1. Email us at enbilex@gmail.com
2. Include "Privacy Request" in the subject line
3. Specify your request type (access/delete/correct)
4. We'll respond within 30 days as required by law

---

## 🌍 Compliance

Our policies comply with:

- ✅ **GDPR** (General Data Protection Regulation - EU)
- ✅ **CCPA** (California Consumer Privacy Act - California, USA)
- ✅ **COPPA** (Children's Online Privacy Protection Act - USA)
- ✅ **Apple App Store Review Guidelines**
- ✅ **US Federal Trade Commission (FTC) Guidelines**

---

## 📱 For App Store Reviewers

This repository contains:
- ✅ Publicly accessible privacy policy (required)
- ✅ Complete terms of service
- ✅ Valid contact information
- ✅ Compliance with App Store Review Guidelines
- ✅ HTTPS-secured pages via GitHub Pages
- ✅ Mobile-responsive design

Privacy Policy URL for App Store Connect:
```
https://gutugut.github.io/calculatorera-policies/privacy.html
```

---

**Last Updated**: November 23, 2025  
**Repository**: calculatorera-policies  
**Contact**: enbilex@gmail.com  
**Region**: United States 🇺🇸

---

*Thank you for using Calculator Era! 🧮✨*
